...
export default function ResumoExecutivoPorUF({
  uf,
  metricas,
  metaROB,
  dadosMesAtual = [],
  ...
  const valorMesAnterior = metricasMesAnterior?.ROB
  const valorAnoAnterior = metricasAnoAnterior?.ROB
  ...
  return (
    <BrandCard variant="elevated" padding="lg">
      <div className="mb-6">
        <h2 className="text-xl font-heading font-black text-neutral-900 dark:text-white mb-2 flex items-center gap-2">
          {getBandeiraPath(uf) ? (
            <img
              src={getBandeiraPath(uf)}
              alt={`Bandeira ${uf}`}
              className="w-9 h-9 rounded-full object-cover flex-shrink-0 ring-2 ring-gray-200 dark:ring-[#404040]"
            />
          ) : (
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-[#0430BA] to-[#3549FC] flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
              {uf}
            </div>
          )}
          Resumo - {uf}
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-6">
        <div>
          ...
          <p className="text-3xl font-display font-black text-neutral-900 dark:text-white mb-2">
      ...
      {/* Métricas Secundárias (inclui MB% e MC%) */}
